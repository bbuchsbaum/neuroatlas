...
vignette: |
  %\VignetteIndexEntry{Atlas Visualization with Optimal Colours} %\VignetteEngine{knitr::rmarkdown} %\VignetteEncoding{UTF-8}
---
...
```{r quick-legend, fig.height = 5}
plot(atlas, legend = TRUE)   # ASEG has 17 regions
```

The legend is capped by `legend_max` (default 30), so it is silently skipped
for large parcellations; raise `legend_max` to override. It is drawn for the
montage view (each orthogonal plane shows a different subset of regions).
