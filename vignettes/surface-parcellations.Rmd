...
Keep the atlas and the data conceptually separate. A numeric vector supplied
to `plot_brain()` must have exactly one value for every element of `atl$ids`:
...
Use the tibble for modelling and reporting; pass the original atlas plus the
same value vector to `plot_brain()` for a surface figure.
