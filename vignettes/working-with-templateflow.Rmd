...
Inspecting `tf` after it is successfully created will show you:
...
```
(If the template list does not appear during build, see the note below.)
...
t1_multi_res[["1"]] # The 1mm NeuroVol
t1_multi_res[["2"]] # The 2mm NeuroVol
```
...
For common tasks, use `get_template()` for volumes and
`get_surface_template()` for surface file paths:

### Volumetric Data with `get_template()`
```{r get_volume_helper, eval = FALSE}
# Get a T1-weighted brain volume
t1_vol <- get_template(
  space = "MNI152NLin2009cAsym",
  variant = "brain",
  modality = "T1w",
...
Example output is abbreviated here because the exact path, data range, and
cache state depend on your local TemplateFlow installation:
...
For masks, probability maps, segmentations, or other modalities, change
`variant`, `modality`, `label`, or `suffix` as shown above.
