...
`neuroim2::plot_overlay()` — the same recipe as the Wang maps above, but with
a dataset that downloads automatically, so the figure below is fully
reproducible:
...
```{r visf-plot, eval = FALSE}
plot(get_visfatlas(), view = "ortho")   # three orthogonal planes
plot(get_visfatlas(), nslices = 8)      # axial montage
```
